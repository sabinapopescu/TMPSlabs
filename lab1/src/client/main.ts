...
function lineToText(line: {kind:string;color:string;price:number;qty:number}){
  return `+ ${line.qty}× ${line.color} ${line.kind} @ ${line.price.toFixed(2)}`;
}

// store added lines so the user sees them before build()
const builderItems: {kind:string;color:string;price:number;qty:number}[] = [];

function buildFromForm(){
  builder.reset();
  builder.setName(($("#name") as HTMLInputElement).value);
  for(const l of builderItems){
    builder.addFlower(l.kind, l.color, l.price, l.qty);
  }
  builder
    .addWrapping(($("#wrapKind") as HTMLSelectElement).value as any, ($("#wrapColor") as HTMLInputElement).value)
    .addRibbon(($("#ribbonMat") as HTMLSelectElement).value as any, ($("#ribbonColor") as HTMLInputElement).value)
    .addCard(($("#cardMessage") as HTMLInputElement).value);
...
function renderPreview(){
  if(!current){ preview.textContent = "No bouquet yet."; return; }
...
$("#addFlower").addEventListener("click", () => {
  const kind = ($("#flowerKind") as HTMLSelectElement).value;
  const color = ($("#flowerColor") as HTMLSelectElement).value;
  const qty = parseInt(($("#flowerQty") as HTMLInputElement).value, 10) || 1;
  const price = parseFloat(($("#flowerPrice") as HTMLInputElement).value) || 1;
...
$$('#settings button[data-currency]').forEach(btn => {
  btn.addEventListener("click", () => {
    cfg.setCurrency((btn as HTMLButtonElement).dataset.currency as any);
    renderBadge();
    if (current) renderPreview(); // recompute label with new currency
  });
});
...
$("#payBtn").addEventListener("click", () => {
  if(!current){ receipt.textContent = "Build a bouquet first."; return; }
  const amount = current.estimate();
  const method = (document.querySelector('input[name="pay"]:checked') as HTMLInputElement).value;

  let out = "";
  if(method === "card"){
    const masked = (document.getElementById("cardMask") as HTMLInputElement).value || "**** **** **** 0000";
    out = payments.checkout(amount, { type: "card", masked });
  } else if(method === "crypto"){
    const network = ((document.getElementById("net") as HTMLSelectElement).value || "ETH") as "ETH"|"BTC"|"TRX";
    const address = (document.getElementById("addr") as HTMLInputElement).value || "0x...";
    out = payments.checkout(amount, { type: "crypto", network, address });
  } else {
    const iban = (document.getElementById("iban") as HTMLInputElement).value || "MD00...";
